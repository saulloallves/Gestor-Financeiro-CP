-- Migration: Corrigir função para fazer cast correto do perfil
-- Data: 2025-09-05

-- Recriar a função com cast correto para o enum existente
...
DECLARE
    ...
    v_perfil_final TEXT;
BEGIN
    -- Mapear perfil para os valores aceitos pelo enum existente
    CASE p_perfil
        WHEN 'admin' THEN v_perfil_final := 'admin';
        WHEN 'gestor' THEN v_perfil_final := 'gestao'; -- Mapear para o enum existente
        WHEN 'juridico' THEN v_perfil_final := 'cobranca'; -- Mapear para o enum existente
        WHEN 'operador' THEN v_perfil_final := 'cobranca'; -- Mapear para o enum existente (operador = cobranca)
        ELSE 
            v_perfil_final := 'cobranca'; -- Default para cobranca
    END CASE;
    ...
    BEGIN
        ...
        -- 2. Criar registro na tabela usuarios_internos fazendo cast para o enum correto
        INSERT INTO usuarios_internos (
            ...
            p_email,
            p_telefone,
            v_perfil_final::perfil_usuario,
            p_equipe_id,
            'ativo', -- Status como text
            NOW(),
            NOW()
        );
...
'Cria um usuário interno com conta de autenticação fazendo cast correto para perfil_usuario enum. Retorna JSON com resultado da operação.';
