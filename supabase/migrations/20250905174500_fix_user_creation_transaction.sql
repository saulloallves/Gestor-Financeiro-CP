-- Migration: Limpar usuários órfãos e melhorar função de criação
-- Data: 2025-09-05

-- Função para limpar usuários órfãos (em auth.users mas não em usuarios_internos)
...
BEGIN
    -- Buscar usuários em auth.users que não estão em usuarios_internos nem em franqueados
    FOR v_user_record IN 
        SELECT au.id, au.email 
        FROM auth.users au
        WHERE au.id NOT IN (
            SELECT COALESCE(ui.user_id, '00000000-0000-0000-0000-000000000000'::uuid) 
            FROM usuarios_internos ui 
            WHERE ui.user_id IS NOT NULL
        )
        AND au.id NOT IN (
            SELECT COALESCE(f.user_id, '00000000-0000-0000-0000-000000000000'::uuid) 
            FROM franqueados f 
            WHERE f.user_id IS NOT NULL
        )
        -- Não deletar usuários muito antigos (mais de 1 dia)
    ...
    LOOP
        ...
        RAISE NOTICE 'Deleted orphan user: % (%)', v_user_record.email, v_user_record.id;
    END LOOP;

    RETURN json_build_object(
        'success', true,
        'deleted_count', v_deleted_count,
        'message', format('Limpeza concluída. %s usuários órfãos removidos.', v_deleted_count)
    );
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN json_build_object(
            'success', false,
            'error', SQLERRM,
            'message', 'Erro ao limpar usuários órfãos'
        );
END;
$$;

-- Melhorar a função de criação de usuários internos
...
-- Comentários
COMMENT ON FUNCTION cleanup_orphan_auth_users IS 
'Remove usuários órfãos da tabela auth.users que não estão vinculados a usuarios_internos ou franqueados';

COMMENT ON FUNCTION create_usuario_interno_with_auth IS 
'Cria um usuário interno com conta de autenticação usando transação segura. Retorna JSON com resultado da operação.';
