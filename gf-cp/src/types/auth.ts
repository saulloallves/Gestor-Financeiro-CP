...
export const loginUnidadeSchema = z.object({
  codigoUnidade: z.string().min(3, 'Código da unidade deve ter no mínimo 3 caracteres'),
  senha: z.string().min(6, 'Senha deve ter no mínimo 6 caracteres'),
});
...
export interface AuthState {
  ...
  isLoading: boolean;
  login: (dados: LoginInternoData | LoginFranqueadoData | LoginUnidadeData, tipo: 'interno' | 'franqueado' | 'unidade') => Promise<void>;
  logout: () => Promise<void>;
